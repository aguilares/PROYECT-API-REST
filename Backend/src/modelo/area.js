...
export class Area {
    constructor(id, nombre, laboratorio) {
        this._id = id;
        this._nombre = nombre;
    ...
    listar = async () => {
        const sql =
        `SELECT id, nombre, laboratorio FROM area ORDER BY id DESC LIMIT 4`;
        const [rows] = await pool.query(sql)
        return rows
    }
    ...
    listarSiguiente = async (id) => {
        const sql =
        `SELECT id, nombre, laboratorio FROM area where id<${pool.escape(id)} ORDER BY id DESC  LIMIT 4 `;
        const [rows] = await pool.query(sql)
        return rows
    }
